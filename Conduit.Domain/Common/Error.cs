...
public sealed record Error(string Code, ErrorType Type, string? Description = null)
{
    ...
    public static Error NotFound(string? description = null) => new("not.found", ErrorType.NotFound, description ?? "The requested resource was not found.");
    
    public static Error Unauthorized(string description) => new("unauthorized", ErrorType.Unauthorized, description);
